...
private:
    ...
    struct node {
        ...
        // constructor causes code to crash this time round...
        //
        // node ()
        // {
        //     data_.store(nullptr);
    ...
    static void increase_external_count(std::atomic<counted_node_ptr> &counter, counted_node_ptr &old_number)
    {
        ...
        } while (!counter.compare_exchange_strong(old_number, new_counter,
                                                  std::memory_order_acq_rel, std::memory_order_relaxed));
    ...
    static void free_external_counter(counted_node_ptr &old_node_ptr)
    {
        ...
        } while (!on_ptr->count_.compare_exchange_strong(old_counter, new_counter,
                                                         std::memory_order_acq_rel, std::memory_order_relaxed));
    ...
    {
        node *current_tail_ptr = old_tail.ptr_;
